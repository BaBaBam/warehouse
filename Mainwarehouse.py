...
class Robot:
   def __init__(self):
       ...
       self.id_product = "    "
       self.ware = 0
   def findProduct(self,product_id1):
       if self.rehash_key[product_id1] != "    ":
           print("Found the product at " + self.rehash_key[product_id1])
       else:
   ...
   def hashing_key(self, id_product):
       ...
       if position < 33100:
           ...
           self.hash_row = int((position - 34600) / 25) % 7
           self.hash_slot = int((position - 34600) % 25)
       else:
           self.hash_warehouse = 5
           self.hash_row = int((position - 35100) / 400) % 20
           self.hash_slot = int((position - 35100) % 400)
   def retrieve(self,id_product):
   ...
   def store(self, id_product):
       ...
       if self.rehash_key[id_product] == "Belt":
           return print("now product " + id_product + " is on belt")
       if self.rehash_key[id_product] != "    ":
...
while(input_command != "stop"):
   ...
   if input_command != "stop":
       if len(input_command) != 5 | len(input_command) != 9:
           ...
           if (ord(product_id1[0]) > 64) and (ord(product_id1[0]) < 90):
               if (ord(product_id1[1]) > 48) and (ord(product_id1[1]) < 53):
                   if (ord(product_id1[2]) > 47) and (ord(product_id1[2]) < 58):
                       if (ord(product_id1[3]) > 47) and (ord(product_id1[3]) < 58):
                           if order == str('0'):
                               r.retrieve(product_id1)
                           if order == str('1'):
                               r.store(product_id1)
                           if order == str('2'):
                               r.sortWarehouse(ord(input_command[1].upper()) - 64, int(input_command[2:4]))
                           if order == str('5'):
                               if r.rehash_key[product_id1] != "    ":
                                   print("Found the product at " + r.rehash_key[product_id1])
                               else:
                                   print("Product not found")
                           if order == str('9'):
                               if len(input_command) == 9:
                                   if (ord(product_id2[0]) > 64) and (ord(product_id2[0]) < 90):
   if (ord(product_id2[1]) > 48) and (ord(product_id2[1]) < 53):
       if (ord(product_id2[2]) > 47) and (ord(product_id2[2]) < 58):
           if (ord(product_id2[3]) > 47) and (ord(product_id2[3]) < 58):

                                                   r.manuallmove(product_id1, product_id2)
                                               else:
                                                   print("position that you want to store is incorrect")
                                           else:
                                               print("position that you want to store is incorrect")
                                       else:
                                           print("position that you want to store is incorrect")
                                   ...
                                   print("your order is incorrect")
                       else:
                           print("id_product is incorrect")
                   else:
                       print("id_product is incorrect")
               else:
                   print("id_product is incorrect")
